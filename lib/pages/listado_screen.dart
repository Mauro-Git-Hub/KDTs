...
class _ListadoScreenState extends State<ListadoScreen> {
  ...
  void initState() {
    super.initState();
    // Llamamos a la función para agregar el nuevo registro usando los parámetros pasados
    _addItem(
      widget.cliente,
    ...
    _getCurrentLocation(); // Actualiza la dirección actual
  }
    ...
    if (pickedDate != null) {
      setState(() {
        _fechaEntregaController.text =
            // pickedDate.toIso8601String().split('T').single;
            _fechaEntregaController.text =
  ...
  Future<void> _addItem(
    ...
    await SQLHelper.createItem(cliente, direccionOrigen, direccionDestino,
        precio, parsedDate, imagenPath, tipo);
    _refreshJournals();
  }
  ...
  Future<void> _updateItem(int id) async {
    ...
    DateTime fechaEntrega =
        DateFormat('yyyy-MM-dd').parse(_fechaEntregaController.text);
